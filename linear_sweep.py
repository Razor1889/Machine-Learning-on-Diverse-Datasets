...
def function():
    ...
    return train_and_test_model(model=model_0, loss_function=loss_function,
                                                                            optimizer=optimizer,
...
if __name__ == '__main__':
    if os.path.exists("speeds.txt"):
        os.remove('speeds.txt')
    START : int = 10000
    COUNT_LOOPS : int = 50 # how many points to plot 
    
    device = 'cpu'
    cpu, cuda = [], []
    for i in range(START, COUNT_LOOPS * START + START, START):
        NUM_DATAPOINTS = i
        cpu.append(function())

    device = 'cuda' 
    for i in range(START, total := COUNT_LOOPS * START + START, START):
        NUM_DATAPOINTS = i
        cuda.append(function())
    
    # Calculate trend lines
    poly_degree = 1
    cpu_coefficients = np.polyfit(range(len(cpu)), cpu, poly_degree)
    cuda_coefficients = np.polyfit(range(len(cuda)), cuda, poly_degree)
    cpu_trend_line = np.poly1d(cpu_coefficients)
    cuda_trend_line = np.poly1d(cuda_coefficients)
    
    # Plotting
    plt.figure(figsize=(10, 6))
    plt.plot(range(len(cpu)), cpu, label='CPU')
    plt.plot(range(len(cuda)), cuda, label='CUDA')
    plt.plot(range(len(cpu)), cpu_trend_line(range(len(cpu))), linestyle='--', label=f'CPU Trend Line: {cpu_trend_line}', alpha=0.7)
    plt.plot(range(len(cuda)), cuda_trend_line(range(len(cuda))), linestyle='--', label=f'CUDA Trend Line: {cuda_trend_line}', alpha=0.7)
    plt.title('Execution Time Comparison (CPU vs CUDA) with Trend Lines')
    plt.xlabel('Number of datapoints in data set (tens of thousands)')
    plt.ylabel('Time (s)')
    plt.legend()
    plt.grid(True)
    plt.show()